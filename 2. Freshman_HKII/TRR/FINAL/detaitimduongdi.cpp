...
int main()
{
    ...
    setbkcolor(15); //xac lap mau nen
  	settextjustify(1, 1);  //xac dinh vi tri van ban viet ra (tam là vi tri con tro, xung quanh con tro)
  	...
  	for (int i=0;i<n;i++)
		for (int j=0;j<n;j++)
			if(time[i][j]<150) 
			{
				line(q[i].x, q[i].y, q[j].x, q[j].y);  //ve duong noi hai diem A(q[i].x, q[i].y) và B(q[j].x, q[j].y)
				//printf("1 ");
			}
				
			
	setcolor(12);
	settextstyle(0,0,1); //thiet lap kieu chu hien ra trên màn hình (font, nam ngang tu trai qua phai, phong to)
